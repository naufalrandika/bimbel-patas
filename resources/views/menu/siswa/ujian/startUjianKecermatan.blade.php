...
@section('container')
    ...
    <script>
        ...
        var totalQuestions = soalData.length;
        var totalCountdown;
        var questionCountdown;
        var isTimeUp = false;
        var currentKecermatanIndex = 0;
        var currentKecermatanId = kecermatanData[currentKecermatanIndex].id;
        var questionsAnswered = 0;

        function displayQuestion(index) {
            if (index < soalData.length) {
                var currentQuestion = soalData[index];
                var kecermatan = kecermatanData.find(k => k.id === currentQuestion.kecermatan_id);
                ...
                // Display ABCDE from kecermatan in kolom-text
                ['a', 'b', 'c', 'd', 'e'].forEach(function(letter) {
                    ...
                    if (kecermatan[letter]) {
                        kolomTextElement.textContent = kecermatan[letter];
                ...
                ['a', 'b', 'c', 'd', 'e'].forEach(function(letter) {
                    ...
                    if (kecermatan[letter]) {
                        answerElement.textContent = kecermatan[letter];
        ...
        function moveToNextKecermatan() {
            ...
            if (currentKecermatanIndex < kecermatanData.length) {
                currentKecermatanId = kecermatanData[currentKecermatanIndex].id;
                soalData = soalData.filter(q => q.kecermatan_id ===
                    currentKecermatanId); // Ensure to get new soalData for next Kecermatan
        ...
        function goToNextQuestion() {
            ...
            if (questionsAnswered % 10 === 0) {
                startQuestionCountdown(); // Reset the timer every 10 questions
            }
            if (soalIndex < totalQuestions - 1) {
                soalIndex++;
                displayQuestion(soalIndex);
            } else {
                moveToNextKecermatan();
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.form-check-input').forEach(function(inputElement) {
                inputElement.addEventListener('change', function(event) {
                    ...
                    var currentQuestion = soalData[soalIndex];
        ...
        function startQuestionCountdown() {
            ...
            var remainingQuestionSeconds = {{ $ujian->time }};
